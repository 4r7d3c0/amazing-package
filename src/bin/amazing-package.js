...
import amazingPackage from '../'

if (_help) {
  const usage = usually({
    description: 'Hello World',
    example: 'amazing-package example.txt -o out.txt',
    line: 'amazing-package input [-o output] [-ihv]',
...
(async () => {
  try {
    ...
    const output = await amazingPackage({
      shouldRun: true,
      text: content,
    })
